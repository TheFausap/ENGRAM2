...
- An image-reading control that sends a local PNG/JPEG/WebP/GIF to a configured vision-capable OpenAI-compatible model using the OpenAI-style image message format. This can use a separate backend URL from the main chat model, so the coder model and the vision model can run on different hardware. A successful image analysis is stored as a visual observation and passed into the next chat turn, allowing the main coder model to reason over what the vision model saw.
